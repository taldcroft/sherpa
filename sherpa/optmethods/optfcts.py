...
def _my_is_nan(x):
    fubar = filter(
        lambda xx: xx != xx or xx is numpy.nan or numpy.isnan(xx) and numpy.isfinite(xx), x)
    if len(fubar) > 0:
        return True
    else:
        return False
...
__all__ = ('difevo', 'difevo_lm', 'difevo_nm', 'grid_search',
           'lmdif', 'minim', 'montecarlo', 'neldermead')
...
def lmdif(fcn, x0, xmin, xmax, ftol=EPSILON, xtol=EPSILON, gtol=EPSILON,
    ...
    info, nfev, fval, covarerr = _minpack.mylmdif(
        stat_cb1, m, x, ftol, xtol, gtol, maxfev, epsfcn, factor, verbose, xmin, xmax)

    if par_at_boundary(xmin, x, xmax, xtol):
        nm_result = neldermead(fcn, x, xmin, xmax, ftol=numpy.sqrt(
            ftol), maxfev=maxfev-nfev, finalsimplex=2, iquad=0, verbose=0)
...
def lmdif_cpp(fcn, x0, xmin, xmax, ftol=EPSILON, xtol=EPSILON, gtol=EPSILON,
    ...
    x, fval, nfev, info, covarerr = _saoopt.cpp_lmdif(
        stat_cb1, m, x, ftol, xtol, gtol, maxfev, epsfcn, factor, verbose, xmin, xmax)
