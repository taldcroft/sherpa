...
class InputErr(OptErr):
    ...
    def __str__(self):
        msg = 'The input arrays: low (%s), x (%s) and high (%s) sizes must match' % (
            self.low, self.x, self.high)
        return msg
...
class OutOfBoundErr(OptErr):
    ...
    def __str__(self):
        msg = 'The following condition must be valid: low (%s) <= x (%s) <= high (%s)' % (
            self.low, self.x, self.high)
        return msg
...
class Polytope(MyOpt):
    ...
    def shrink(self, shrink_coef, npar):
        ...
        for ii in xrange(1, npars_plus_1):
            self.polytope[ii] = self.polytope[0] + shrink_coef * \
