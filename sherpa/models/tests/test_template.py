...
class test_new_templates_ui(SherpaTestCase):
    ...
    # TestCase 3.1 discrete templates fail when probed for values they do not
    # represent (gridsearch with wrong values)
    @needs_data
    def test_load_template_model_without_interpolation(self):
        try:
            self.run_thread(
                'load_template_without_interpolation', scriptname='test_case_2_and_3.1.py')
        except ModelErr:
            return
        self.fail('Fit should have failed: using gridsearch with wrong parvals')

    # TestCase 3.2 discrete templates fail when probed for values they do not
    # represent (continuous method with discrete template)
    @needs_data
    def test_load_template_model_without_interpolation(self):
        try:
